    ...
    .map(
      (item) => `<div class="product">
          ...
          <div class="product-info">
            <h2>${item.title}</h2>
            <p>$${item.price}</p>
            <a data-id="${item.id}" class="add-to-cart">Add to cart</a>
          </div>
        </div>`,
    )
    .join("");

  //oluşturulan html'İ arayüze aktar
  uiElements.productList.innerHTML = productsHtml;

  // Add to cart elemanlarına eriş
  const addToCartButtons = document.querySelectorAll(".add-to-cart");
  //addToCartButtons dizisi içerisindeki her bir elemana erişmek için forEach ile diziyi döneriz
...
const renderCartItems = (cart) => {
  ...
  // removeButtons içerisindeki her bir elemana ayrı ayrı eriş ve addEventListener ekle
  ...
  // quantityInputs'larının her birine olay izleyicisi ekle
